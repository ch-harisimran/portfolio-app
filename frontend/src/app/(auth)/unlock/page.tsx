...
import { promptNativeBiometric } from "@/lib/biometric";
...
export default function UnlockPage() {
  ...
  const unlockWithBiometric = async () => {
    if (!isNativeAndroid || !user?.email) return;
    setBioLoading(true);
    try {
      const ok = await promptNativeBiometric();
      if (!ok) throw new Error("Biometric unavailable");
      await authApi.saveDevice({ device_id: deviceId, platform: "android", label: navigator.userAgent, biometric_enabled: true });
      const { data: challenge } = await authApi.createPublicChallenge(user.email, "biometric", deviceId);
      const { data: tokens } = await authApi.biometricUnlock({ email: user.email, challenge_id: challenge.challenge_id, assertion: `bio-${Date.now()}`, device_id: deviceId });
      saveTokens(tokens.access_token, tokens.refresh_token);
      await authApi.sessionUnlock(user.email, deviceId);
      setPinUnlocked(true);
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            {isNativeAndroid && (
              ...
              >
                ...
                {bioLoading ? "Authenticating..." : "Face / Fingerprint"}
              </button>
            )}
